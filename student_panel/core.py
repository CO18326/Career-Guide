...
def get_course_tag_list_student_interest(skill_name : str) -> List[CourseTagMapObject]:
...
def get_professor_tag_list(course_list, skill_name : str): 
    tag_list = []
    # fetch course object list. 
    list_of_hot_words = SkillHotWords.objects.filter(skill_name = skill_name) # list of all host workds. 
    ...
    for course in course_list: 
        ...
        if course_skill_map.is_required(): 
            tag_list.append(skill_name)

    return tag_list # return list of targeted course with tag. 
